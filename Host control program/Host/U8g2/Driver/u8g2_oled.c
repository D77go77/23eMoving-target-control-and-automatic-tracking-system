...
//    u8g2_DrawFrame(u8g2,5,24,18,16);  //画空心圆形
	...
	sprintf_(tmp,"point1:%d,%d", Box_POS[3][0],Box_POS[3][1]);
	

	sprintf_((char *) Buffer1, "Mode:%d", Mode);
	u8g2_DrawStr(u8g2, 10, 12, (char *) Buffer1);
	u8g2_DrawStr(u8g2, 10, 50, tmp);
	
	if (Mode == 1) 
	{ // 校准原点
		if (Mode_1_XY == 0)
			sprintf_((char *) Buffer1, "calibrate X");
		else
			sprintf_((char *) Buffer1, "calibrate Y");
		u8g2_DrawStr(u8g2, 10, 26, (char *) Buffer1);
	}
	if (Mode == 2) 
	{ // 校准边角
		sprintf_((char *) Buffer1, "dis:%dmm", Screen_Dis);
		u8g2_DrawStr(u8g2, 10, 26, (char *) Buffer1);
	}
	if (Mode == 3) 
	{ // 画大框框
		sprintf_((char *) Buffer1, "time:%dms", Pulse_Time);
		u8g2_DrawStr(u8g2, 10, 26, (char *) Buffer1);
		if (Stop_Flag == 1) {
			sprintf_((char *) Buffer1, "STOP!");
			u8g2_DrawStr(u8g2, 10, 40, (char *) Buffer1);
		}
	}
	if (Mode == 4) 
	{ // 画小框框
	...
	if (Mode == 5) 
	{ // 画小框框
		sprintf_((char *) Buffer1, "IMG:%d", IMG_Chose);
		u8g2_DrawStr(u8g2, 10, 26, (char *) Buffer1);
	}
