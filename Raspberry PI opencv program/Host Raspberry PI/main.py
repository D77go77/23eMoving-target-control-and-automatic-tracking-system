...
ser = serial.Serial('/dev/ttyAMA0',9600) # 串口初始，根据实际情况修改串口号和波特率 
...
def key_deal():
    ...
    key_control()
    if get_key == S7 and STATE != 4:# watch the video
        trggle = not trggle
    ...
    if trggle == 1:
        ...
        if STATE == 0:
            ...
            if point_4_ok == 1:
                ...
                if (big_ret_4_point[0][0] == None) or (big_ret_4_point[0][1] == None) or (big_ret_4_point[1][0] == None) or (big_ret_4_point[1][1] == None) or(big_ret_4_point[2][0] == None) or (big_ret_4_point[2][1] == None) or(big_ret_4_point[3][0] == None) or (big_ret_4_point[3][1] == None) :
                    STATE = 1
                    show_scan = 1
                else:
             ...
             if show_scan == 1:
                ...
                num = f"B{big_ret_4_point[0][0]},{big_ret_4_point[0][1]},{big_ret_4_point[1][0]},{big_ret_4_point[1][1]},{big_ret_4_point[2][0]},{big_ret_4_point[2][1]},{big_ret_4_point[3][0]},{big_ret_4_point[3][1]},{center_of_rectangle[0]},{center_of_rectangle[1]},F"
            ...
            if find_small_res == 1:
                ...
                if approx == None:
                    ...
                    draw = ImageDraw.Draw(image)
                    draw.text((10, 0), "Task  2", font=setFont, fill=255, direction=None)
                    draw.text((40, 20), "None", font=setFont, fill=255, direction=None)
                    drawImg(image)
                    num = "NF"
                    ser.write(str(num).encode())
                else:
                    image = Image.new('RGB', (160, 128))
                    image = Image.open("img_small_rec.jpg")
                    image = image.convert('RGBA')
                    image = image.resize((160, 128))  #也可以从地址读取图片文件，并缩放为160x128
                    draw = ImageDraw.Draw(image)
                    setFont = ImageFont.load_default()
                    ...
                    draw.text((10, 0), "Task  2", font=setFont, fill=255, direction=None)
                    drawImg(image)
                    x00,y00=approx[0][0][0]
                    x01,y01=approx[0][3][0]
                    x02,y02=approx[0][2][0]
                    x03,y03=approx[0][1][0]
                    x10,y10=approx[0][0][0]
                    x11,y11=approx[0][3][0]
                    x12,y12=approx[0][2][0]
                    x13,y13=approx[0][1][0]
                    num = f"S{x00},{y00},{x01},{y01},{x02},{y02},{x03},{y03},{x10},{y10},{x11},{y11},{x12},{y12},{x13},{y13},F"
                    print(num)
                    ser.write(str(num).encode())
                find_small_res = 0
                cap.release()
